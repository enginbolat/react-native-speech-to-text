...
final class SpeechRecognitionEngine {
  ...
  func startListening(
    ...
    didSettlePromise = true // Don't reject in teardown when we're just cleaning up from a previous session
    teardown()
    onTeardown = onTeardownComplete
    didSettlePromise = false
    sessionId = UUID().uuidString

    guard audioSessionManager.captureAndConfigure() else {
      didSettlePromise = true
      let err = NSError(domain: "SpeechRecognition", code: -1, userInfo: [NSLocalizedDescriptionKey: "Audio session configuration failed"])
      onStartError?(err)
      notifyError(callbacks?.onError, code: "audio", message: "")
      teardown()
      return
    }
  ...
  func startTranscription(
    ...
    speechRecognizer = makeRecognizer(locale: locale)
    let url = URL(fileURLWithPath: filePath)
    urlRequest = SFSpeechURLRecognitionRequest(url: url)
    applyOptions(from: options, to: urlRequest)

    guard let request = urlRequest else {
      notifyError(callbacks?.onError, code: "recognition_url_init", message: "")
      teardown()
      return
    }
  ...
  private func handleSpeechResult(
    ...
    if let error = error {
      reportErrorAndTeardown(error: error, notify: callbacks?.onError)
      return
    }
    guard let result = result else {
      reportEndAndTeardown(callbacks?.onEnd)
      return
    }
    ...
    if result.isFinal || recognitionTask?.state == .canceling || recognitionTask?.state == .finishing {
      reportEndAndTeardown(callbacks?.onEnd)
    }
  }
  ...
  private func installTapAndStartEngine(inputNode: AVAudioInputNode, callbacks: SpeechCallbacks?) {
    ...
    do {
      ...
      didSettlePromise = true
      onStartError?(error)
      notifyError(callbacks?.onError, code: "audio", message: error.localizedDescription)
      teardown()
    }
  }
